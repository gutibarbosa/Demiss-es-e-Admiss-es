 ...
 *
 * v3: tabela de colaboradores recebe 3 novas colunas operacionais:
 *   — Status do Processo  (lido de STATUS_DESLIGAMENTO / STATUS_ADMISSÕES)
 *   — Progresso           (barra visual + ícone derivado do status)
 *   — Dias em Aberto      (calculado a partir da data de referência)
 * Novo filtro de processo na barra de controles da tabela.
 * Busca textual estendida para incluir o campo statusProcesso.
 */
...
function detectarColunas(keys, aliasMap) {
  const map = {};
  for (const [c, aliases] of Object.entries(aliasMap)) {
    for (const key of keys) {
      if (aliases.includes(normStr(key))) { map[c] = key; break; }
    }
  }
  return map;
}
...
/**
 * Tenta fazer parse de uma data em DD/MM/YYYY ou YYYY-MM-DD.
 * Retorna um objeto Date ou null.
 */
...
/** Calcula quantos dias se passaram desde dataStr até hoje. */
...
function normalizarDesligamentos(rows) {
  ...
  return rows.flatMap(row => {
    ...
    const dataRef        = campo(row, col.dataReferencia) || campo(row, col.dataDesligamento) || '';
    ...
    return [{ colaborador, loja, supervisor, funcao, regiao,
              status: 'DEMISSÃO', mes: mesRaw, ano,
              statusProcesso, dataRef }];
  });
}
...
function normalizarAdmissoes(rows) {
  ...
  return rows.flatMap(row => {
    ...
    const mesRaw         = campo(row, col.mes).toUpperCase().substring(0, 3);
    const statusProcesso = campo(row, col.statusProcesso) || '';
    const dataRef        = campo(row, col.dataReferencia) || campo(row, col.dataAdmissao) || '';
    ...
    return [{ colaborador, loja, supervisor, funcao, regiao,
              status: 'ADMISSÃO', mes: mesRaw, ano,
              statusProcesso, dataRef }];
  });
}
...
function processarBase(rows) {
  ...
  if (rows[0].COLABORADOR !== undefined || rows[0].colaborador !== undefined) {
    const col = detectarColunas(Object.keys(rows[0]), {
      ...
      ano:             ['ano'],
      status:          ['status'],
      statusProcesso:  ['status_desligamento','status_demissao','status_admissoes','status_admissao','status_processo'],
      dataRef:         ['ultimo_dia_trabalho','data_desligamento','admissao','solicitacao'],
    });
    ...
    for (const row of rows) {
      ...
      State.turnoverData.push({ colaborador, loja, supervisor, funcao, regiao,
                                status, mes: mesRaw, ano, statusProcesso, dataRef });
    }
  } else {
    const seen = new Set();
    State.turnoverData = [];
...
function aplicarFiltroDetalhe() {
  ...
  // Filtro por tipo (ADMISSÃO / DEMISSÃO)
  if (status) rows = rows.filter(r => r.status === status);

  // Filtro por categoria de processo
  if (proc) {
    rows = rows.filter(r => {
  ...
  // Busca textual — inclui statusProcesso
  ...
  rows = [...rows].sort((a, b) => {
    ...
    if (sortField === 'periodo') {
      ...
      va = diasEmAberto(a.dataRef) ?? 9999; vb = diasEmAberto(b.dataRef) ?? 9999;
    } else {
...
function renderDetalheTabela() {
  ...
  if (!slice.length) {
    tbody.innerHTML = `<tr><td colspan="10" class="detalhe-empty"><span>🔍</span>Nenhum colaborador encontrado.</td></tr>`;
  } else {
    tbody.innerHTML = slice.map(r => {
      ...
      const statusProcessoHTML = temStatus
        ? `<span class="badge-processo badge-proc-${interp.categoria.toLowerCase()}" title="${interp.label}">${interp.label}</span>`
        : `<span class="badge-processo badge-proc-sem">—</span>`;
      ...
      const progressoHTML = temStatus
        ? `<div class="prog-wrap" title="${interp.pct}% concluído">
             ...
             <div class="prog-bar-track">
               <div class="prog-bar-fill prog-fill-${interp.categoria.toLowerCase()}" style="width:${interp.pct}%"></div>
             </div>
      ...
      if (dias !== null) {
        let alertCls = '';
        let alertTip = '';
        if (dias > 10) { alertCls = 'dias-critico'; alertTip = 'Crítico — acima de 10 dias'; }
        else if (dias > 5) { alertCls = 'dias-atencao'; alertTip = 'Atenção — entre 6 e 10 dias'; }
        else { alertCls = 'dias-normal'; alertTip = 'Normal — até 5 dias'; }
        diasHTML = `<span class="dias-badge ${alertCls}" title="${alertTip}">${dias}d</span>`;
      }
      ...
      return `<tr>
        ...
        <td class="td-status-proc">${statusProcessoHTML}</td>
...
function setupDetalheListeners() {
  ...
  // ── NOVO: filtro por categoria de processo ──
  ...
  if (processoFilter) processoFilter.addEventListener('change', () => {
    DetalheState.processoFilt = processoFilter.value; DetalheState.page = 1; aplicarFiltroDetalhe();
  });
...
// ══════════════════════════════════════════════════════════════════════════════
// EXPORT CSV — atualizado com novas colunas
// ══════════════════════════════════════════════════════════════════════════════
function exportarCSV() {
  const rows = DetalheState.filtered;
  if (!rows.length) { showToast('Nenhum dado para exportar.', 'warning'); return; }
  const header = ['Colaborador','Função','Loja','Supervisor','Região','Período','Status','Status do Processo','Progresso (%)','Dias em Aberto'];
